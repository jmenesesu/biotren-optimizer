...
def _cantones_via(linea):
    """[(lo, hi, tipo)] cantones de la linea con su tipo (doble/unica)."""
    import pandas as _pd
    bl = _pd.read_csv(CLEAN / "bloques.csv")
    tv = _pd.read_csv(CLEAN / "tramos_via.csv")
    tvl = tv[tv.linea == linea]
    out = []
    for r in bl[bl.linea == linea].itertuples():
        mid = (r.dist_lo + r.dist_hi) / 2
        row = tvl[(tvl.km_lo <= mid) & (tvl.km_hi >= mid)]
        tipo = row.iloc[0].tipo if len(row) else "unica"
        out.append((float(r.dist_lo), float(r.dist_hi), "doble" if tipo == "doble" else "unica"))
    return sorted(out)


def _via_key(sentido, tipo):
    if tipo != "doble":
        return "U"
    ...
    for linea in ["L2", "L1"]:
        ...
        for u, sv in unidades.items():
            for s in sv:
                ...
                for (lo, hi, tipo) in cant:
                    ta = _t_en_pts(s["pts"], lo); tb = _t_en_pts(s["pts"], hi)
                    if ta is None or tb is None:
                        continue
                    t_in, t_out = min(ta, tb), max(ta, tb)
                    entry = lo if ta <= tb else hi
                    vk = _via_key(s["sentido"], tipo)
                    items.setdefault((lo, hi, vk), []).append([t_in, t_out, s["sentido"], tid, entry])
            ...
            for k in range(len(lst)):
                ...
                for j in range(k):
                    ...
                    if z_out > t_in - 1e-6:
                        hu = max(hu, z_out)
                if hu > t_in + 0.01:
                    holds.setdefault(tid, {})[(key[0], key[1])] = (hu, entry)
    return holds
